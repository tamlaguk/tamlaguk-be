...
import tamlagukbe.tamlagukbe.Review.repository.ActivityReviewRepository;
...
public class ActivityReviewService {
    ...
    public ActivityReview transcribe(MultipartFile audioFile, Long userId, Long activityStoreId) throws IOException {
        // S3에 음성 파일 저장 -> URL 반환
        ...
        // Member 및 Activity 엔터티를 가져오기
        Member member = memberRepository.findById(userId)
                .orElseThrow(() -> new RuntimeException("Member not found"));
        Activity activity = activityRepository.findById(activityStoreId)
                .orElseThrow(() -> new RuntimeException("Activity not found"));

        // 결과 저장
        ActivityReview review = new ActivityReview();
        review.setUserId(member);
        review.setActivityId(activity); // Activity 엔터티를 설정
    ...
    public ActivityReview getReviewById(Long reviewId) {
        return activityReviewRepository.findById(reviewId).orElseThrow(() -> new RuntimeException("Review not found"));
    }
